...
with rec {
  ...
  hsPkgs = { cabalPath, cabalCheck, callPackage }: {
    ...
    GetDeps = cabalCheck <get-deps> (fetchFromGitHub {
      owner  = "Warbo";
      repo   = "getDeps";
    ...
    HS2AST = cabalCheck <hs2ast> (fetchFromGitHub {
      owner  = "Warbo";
      repo   = "hs2ast";
    ...
    ifcxt = cabalCheck <ifcxt> (fetchFromGitHub {
      owner  = "mikeizbicki";
      repo   = "ifcxt";
    ...
    ML4HSFE = cabalCheck <ml4hsfe> (fetchFromGitHub {
      owner  = "Warbo";
      repo   = "ml4hsfe";
    ...
    mlspec = cabalCheck <mlspec> (fetchFromGitHub {
      owner  = "Warbo";
      repo   = "mlspec";
      rev    = "a5f0dd2";
    ...
    mlspec-helper = cabalCheck <mlspec-helper> (fetchFromGitHub {
      owner  = "Warbo";
      repo   = "mlspec-helper";
    ...
    nix-eval = cabalCheck <nix-eval> (fetchFromGitHub {
      owner  = "Warbo";
      repo   = "nix-eval";
      rev    = "53d18a2";
...
rec {
  hsOverride = self: super: (hsPkgs rec {
    inherit (self) callPackage;
    cabalPath  = p: self.callPackage (nixFromCabal (toString p) null);
    cabalCheck = path: fallback: cabalPath (if stable
                                               then fallback
                                               else path);
  });
