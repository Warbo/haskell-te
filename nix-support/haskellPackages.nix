...
with rec {
  ...
  optimise = pkg: haskell.lib.overrideCabal pkg (oldAttrs: {
    configureFlags = [ "--ghc-option=-O2" ];
  });

  hsPkgs = { cabalPath, cabalCheck, callPackage }: {
    AstPlugin = cabalCheck "ast-plugin" <ast-plugin> (fetchFromGitHub {
      owner  = "Warbo";
      repo   = "ast-plugin";
    ...
    GetDeps = cabalCheck "get-deps" <get-deps> (fetchFromGitHub {
      owner  = "Warbo";
      repo   = "getDeps";
    ...
    HS2AST = cabalCheck "hs2ast" <hs2ast> (fetchFromGitHub {
      owner  = "Warbo";
      repo   = "hs2ast";
    ...
    ifcxt = cabalCheck "ifcxt" <ifcxt> (fetchFromGitHub {
      owner  = "mikeizbicki";
      repo   = "ifcxt";
    ...
    ML4HSFE = cabalCheck "ml4hsfe" <ml4hsfe> (fetchFromGitHub {
      owner  = "Warbo";
      repo   = "ml4hsfe";
    ...
    mlspec = cabalCheck "mlspec" <mlspec> (fetchFromGitHub {
      owner  = "Warbo";
      repo   = "mlspec";
      rev    = "a5f0dd2";
    ...
    mlspec-helper = cabalCheck "mlspec-helper" <mlspec-helper> (fetchFromGitHub {
      owner  = "Warbo";
      repo   = "mlspec-helper";
    ...
    nix-eval = cabalCheck "nix-eval" <nix-eval> (fetchFromGitHub {
      owner  = "Warbo";
      repo   = "nix-eval";
      rev    = "53d18a2";
    ...
    reduce-equations = cabalCheck "reduce-equations" <reduce-equations>
      (fetchFromGitHub {
        owner  = "Warbo";
        ...
        haskell-src-exts = callPackage (callHackage "haskell-src-exts"
                                                         "1.19.0") {
          pretty-show = callPackage (callHackage "pretty-show" "1.6.12") {
          };
        };
    };

    runtime-arbitrary = cabalCheck "runtime-arbitrary" <runtime-arbitrary>
      (fetchFromGitHub {
        owner  = "Warbo";
...
rec {
  hsOverride = self: super: mapAttrs (_: optimise) (hsPkgs rec {
